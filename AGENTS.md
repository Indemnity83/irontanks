...
On the `mc/26.2` line Iron Tanks is **standalone** — no BuildCraft dependency, and every integration
...
`mc/26.2`. For work targeting **any other `mc/*` line**, check out that line's worktree and use
`git town append` instead — it parents to the *current* branch, which is exactly what you want:
...
1. Create a feature branch with **Git Town** so it is parented correctly — don't use a bare
   ...
   - targeting any other `mc/*` line → from that line's worktree,
...
- **`sync_translations.yml`** — Crowdin round-trip: uploads `en_us.json` on push and opens a
  ...
  branch**, and its push filter is branch-pinned — so this workflow serves exactly one `mc/*` line.
